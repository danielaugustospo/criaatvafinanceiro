...
@isset($pedido)
    <div class="row">
        ...
        <input class="col-sm-1 form-control" type="text" name="id" id=""
            ...
            value="@isset($pedido->id) {{ $pedido->id }} @endisset" {{ $variavelDisabledNaView }} readonly>
    </div>
@endisset
...
<div class="row">
    ...
    <input class="ml-2 mr-2" type="radio" name="oscriaatva" @if (old('ped_os') == 'EMPRESA GERAL' || old('oscriaatva') == 'EMPRESA GERAL') checked @endif
        @isset($pedido) @if ($pedido->ped_os == 'EMPRESA GERAL') checked  @endif @endisset name="ped_os"
        id="compraCRIAATVA" value="EMPRESA GERAL" {{ $variavelDisabledNaView }}>
    ...
    <input class="mr-2" type="radio" name="oscriaatva" @if (old('ped_os') == 'OS' || old('oscriaatva') == 'OS') checked @endif
        @isset($pedido) @if ($pedido->ped_os != 'EMPRESA GERAL') checked @endif @endisset name="ped_os"
        id="compraOS" value="OS" {{ $variavelDisabledNaView }}>
        ...
        <select class="selecionaComInput col-sm-3 form-control" id="ped_banco" name="ped_banco"
            ...
            @foreach ($listaBancos as $bancos)
                <option @if (old('ped_banco') == $bancos->id) selected @endif value="{{ $bancos->id }}"
                    @isset($pedido) @if ($pedido->ped_banco == $bancos->id) selected @endif @endisset>
                    {{ $bancos->codigoBanco }} - {{ $bancos->nomeBanco }}</option>
            @endforeach
        </select>
