...
var _exportData = function ( dt, inOpts )
{
    var config = $.extend( true, {}, {
        indice_do_array_agrupado_um: false,
        rows:           null,
        columns:        '',
        modifier:       {
            search: 'applied',
    ...
    var indice_do_array_agrupado_um   = config.indice_do_array_agrupado_um;
    var indice_do_array_agrupado_dois = config.indice_do_array_agrupado_dois;
    // var indice_do_array_agrupado_tres = config.indice_do_array_agrupado_tres;
    
	var posicao_um = config.posicao_um;
    var posicao_dois = config.posicao_dois;
    // var posicao_tres = config.posicao_tres;
	
    if ( indice_do_array_agrupado_um !== false ) { //don't run grouping logic if rows aren't grouped
        ...
        for (var i = 0, row_length = body.length; i < row_length; i++) {
            var sGroup_um 	= row_data_array[i][indice_do_array_agrupado_um];
            var sGroup_dois = row_data_array[i][indice_do_array_agrupado_dois];

			// console.log(row_data_array[i][indice_do_array_agrupado_dois] + row_data_array[i][indice_do_array_agrupado_um]);

			var indiceanterior = row_data_array;
			var valor = i - 1;
...
if (valor > -1){
	...
	for (var $column_index = 0; $column_index < iColspan; $column_index++) {
			...
			if((row_data_array[i].idFornecedor) == (indiceanterior[valor].idFornecedor) ){
				var somainicial = parseFloat(row_data_array[i].precoReal);						//6.00
				var somaatual =  somainicial[i] + parseFloat(indiceanterior[valor].precoReal);		//12
				// var somaatual = somainicial + parseFloat(row_data_array[i].precoReal);
				// somaatual += parseFloat(indiceanterior[valor].precoReal) + parseFloat(row_data_array[i].precoReal);
				console.log(somaatual);
			}
}			
sGroup_dois = somaatual;
			// console.log(row_data_array[i].idFornecedor);
			// console.log(teste[valor]);

			// if (row_data_array[i][idFornecedor] )

            if ( sGroup_um !== sLastGroup ) {
                var table_data = [];
  
                for (var $column_index = 0; $column_index < iColspan; $column_index++) {
                    if (($column_index === posicao_um) && (posicao_um != null))
                    {
                        // strips anything inside < > tags. hoping this won't be an issue in the future.
                        table_data[$column_index] = sGroup_um.replace( /<[^>]*>/gi, '' ); + " ";
                    }
                    else if ($column_index === posicao_dois)
                    {
                        //table_data[$column_index] = sGroup_dois.replace( /<[^>]*>/gi, '' ); + " ";
                        table_data[$column_index] = sGroup_dois; + " ";
                    }
                    // else if (($column_index === posicao_tres) && (posicao_tres != null))
                    // {
                    //     table_data[$column_index] = sGroup_tres.replace( /<[^>]*>/gi, '' ); + " ";
                    // }
                    else
                    {
